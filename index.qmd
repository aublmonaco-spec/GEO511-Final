...
1.  I installed leaflet and leaflet.extras for making maps, along with rinat and rnpn to download data from iNaturalist and the US National Phenology Network, respectively. I then loaded my necessary libraries. I used dplyr to mutate and filter data, lubridate to parse date data, and ggplot2 for plotting.
...
2.  I loaded my data from iNaturalist and the US National Phenology Network.
...
3.  I combined the datasets from each year into one set. I  converted this data into a csv so I do not have to download the data each time I run the code, and loaded it into my environment as "nat_flower". I filtered nat_flower to only include wild sightings of the selected species (captive_cultivated = "false").
...
4.  I used the rnpn package to download data for the plant species from the National Phenology Network (which automatically saves as a csv). The species IDs were available through the NPN's website (Red Buckeye = 784, columbine = 147).
...
5.  I combined my iNaturalist and NPN data into one dataset.
...
7.  I found the mean date of first sightings of the selected flowers from 2005 to 2013 and from March to June (when these flowers bloom; fl_early dataset) to compare data from the last 11 years. I separated sightings into 3 bands of latitude (to account for few observations).
...
8.  I created the dataset to see trends between 2014 and 2025, and in months March to June. I added a column (mean0013) with the means of the fl_early observations based on latitude. I separated the 3 early bands into 6, using the same mean for 2 bands. I also created a difference column, showing the difference in days of arrival between the 2014 and 2025 and the 2005 to 2013 average using mean - mean0013. I rejoined all of these bands into one dataset for plotting.
...
9.  I followed the same steps to find data for Ruby-throated Hummingbird spring migration and found their average arrival dates between 2005 and 2013. I used data from eBird and Journey North in addition to iNaturalist and the US National Phenology Network.
...
```
Here is my initial combining of iNaturalist data and the NPN download:
...
```
Here is my code for finding the early Ruby-throat arrival dates:
```{r, message=FALSE, warning=FALSE}
...
10. I then similarly created the Ruby-throat dataset for 2014-2025. I again combined these bands into one dataset, and created a difference column, showing the difference in days of arrival between the new dataset and the 2005 to 2013 average using mean - mean0013.
...
11. I created color palettes using viridis scales and colorNumeric from the Leaflet package for my two maps, specifying to use the difference column in each dataset to map the colors to.
