...
#fl_years <- levels(as.factor(2005:2025))
#rb <- "Aesculus pavia"

#nat_flower <- foreach(y = unique(fl_years), n = rb, .packages = c("rinat", "dplyr"), .combine = rbind) %dopar% { get_inat_obs(year = y, taxon_name = n, place_id = 1, maxresults = 5000, quality = "research")
#}
...
fl_early <- all_flower %>%
  filter(year %in% c(2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014) & month %in% c(3, 4, 5, 6))
...
flower_new <- all_flower %>%
  ...
  mutate(mean0014 = case_when(
                          latitude > 30 & latitude < 33 ~ 95.725,
                          latitude > 33 & latitude < 36 ~ 95.725,
                          latitude > 36 & latitude < 39 ~ 98.94444,
                          latitude > 39 & latitude < 42 ~ 98.94444,
                          latitude > 42 & latitude < 45 ~ 121.32,
...
- 30-33: April 21st (About 15 days later)

- 33-36: April 19th (About 13 days later)

- 36-39: April 20th (About 10 days later)

- 39-42: April 20th (About 11 days later)

- 42-45: April 20th (About 12 days earlier)

- 45-50: April 21st (About 20 days earlier)
...
- 30-33: April 23rd (About 3 days later)

- 33-36: May 3rd (About 1 day later)

- 36-39: May 20th (About 1 day earlier)

- 39-42: May 24th (About 4 days earlier)

- 42-45: May 27th (About 1 day earlier)

### Average Bloom Time of Columbine and Red Buckeye (2015-2025)
```{r}

ggplot() + geom_point(data = fl_new_all, aes(x = mean, y = latitude)) + geom_smooth(data = fl_new_all, aes(x = mean, y = latitude), method = "lm", color = "red") + labs(title = "Average Bloom Time of Columbine and Red Buckeye", x = "Mean Bloom Date (Year Days)", y = "Latitude")

```


### Average First Sighting Date of Ruby-throated Hummingbirds (2015-2025)
```{r}

ggplot() + geom_point(data = rubythroat_all, aes(x = mean, y = Latitude)) + geom_smooth(data = rubythroat_all, aes(x = mean, y = Latitude), method = "lm", color = "blue") + labs(title = "Average First Sighting Date of Ruby-throated Hummingbirds", x = "Mean Sighting Date (Year Days)", y = "Latitude")
