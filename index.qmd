...
rth_early <- ruby_throat %>%
  filter(Year %in% c(2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017) & Month %in% c(3, 4, 5))
...
10. I then similarly created the Ruby-throat dataset for 2015-2025, but I added 3 more bands of latitude to account for the larger number of observations recorded at higher and lower latitudes. I again combined these bands into one dataset, and created a difference column, showing the difference in days of arrival between the new dataset and the 2000 to 2017 average using mean - mean0017.

```{r, message=FALSE, warning=FALSE}

rth_new_all <- ruby_throat %>%
  filter(Year %in% c(2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025) & Month %in% c(3, 4, 5)) %>%
  mutate(mean0017 = case_when(Latitude > 30 & Latitude < 33 ~ 95.95455,
                              Latitude > 33 & Latitude < 36 ~ 99.19231,
                              Latitude > 36 & Latitude < 39 ~ 112.1765,
                              Latitude > 39 & Latitude < 42 ~ 133.2071,
                              Latitude > 42 & Latitude < 45 ~ 139.9383
                              ))
